...
Adafruit_SI1145::Adafruit_SI1145(uint8_t addr) {
  _addr = addr;
}
...
uint8_t  Adafruit_SI1145::read8(uint8_t reg) {
  uint16_t val;
    cc_wire_beginTransmission(_addr);
    cc_wire_write((uint8_t)reg);
    cc_wire_endTransmission(false);

    cc_wire_requestFrom((uint8_t)_addr, (uint8_t)1);
    return cc_wire_read();
}
...
uint16_t Adafruit_SI1145::read16(uint8_t a) {
  ...
  cc_wire_beginTransmission(_addr); // start transmission to device
  cc_wire_write(a); // sends register address to read from
  cc_wire_endTransmission(false); // end transmission
  
  cc_wire_requestFrom(_addr, (uint8_t)2);// send data n-bytes read
  ret = cc_wire_read(); // receive DATA
  ret |= (uint16_t)cc_wire_read() << 8; // receive DATA

  return ret;
}

void Adafruit_SI1145::write8(uint8_t reg, uint8_t val) {
	obcl_writeByteToRegAddr(_addr,reg,val);
}
