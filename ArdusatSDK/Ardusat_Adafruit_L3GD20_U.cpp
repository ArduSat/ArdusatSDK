...
void Adafruit_L3GD20_Unified::write8(byte reg, byte value) {
	obcl_writeByteToRegAddr(L3GD20_ADDRESS, reg, value);
	/*
...
byte Adafruit_L3GD20_Unified::read8(byte reg) {
	uint8_t ret = 0;
	obcl_readByteFromRegAddr(L3GD20_ADDRESS, reg, &ret);
	return (ret);
	/*
	 byte value;
...
void Adafruit_L3GD20_Unified::getEvent(sensors_event_t* event) {
	...
	while (!readingValid) {
		...
		obcl_readByteArrayFromRegAddr( L3GD20_ADDRESS,
				(GYRO_REGISTER_OUT_X_L | 0x80), (uint8_t*) &_data, 6, &recdlen);
...
//    while (Wire.available() < 6);
		...
		event->gyro.x = (int16_t) (_data[0] | (_data[1] << 8));
		event->gyro.y = (int16_t) (_data[2] | (_data[3] << 8));
		event->gyro.z = (int16_t) (_data[4] | (_data[5] << 8));
